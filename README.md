...
- **v0.4.1** - Bug修复版本
  - 修复 `ScrollPanel` 类缺失导致的导入错误
  - 修复 `show_message` 和 `show_confirm` 函数未定义的导入错误
  - 代码质量改进和架构优化
...
- **v0.4.0** - 外交系统版本
  ...
  - 实现同盟功能（提议同盟、建立同盟关系）
  - 实现劝降功能（对弱势力进行劝降）
  - 实现外交操作（赠送礼物改善关系、断绝关系、宣战）
  - 修复 PyInstaller 打包后的模块导入问题
  - 更新 GitHub Actions 配置解决 Node.js 20 弃用警告
  - 游戏平衡调整（降低收入、增加消耗、调整内政成本）

- **v0.3.1** - 功能完善版本
  - 新增武将信息界面，点击城市场景中的武将可查看详情
  - 武将属性展示（武力、智力、政治、魅力、忠诚度）
  - 武将操作功能（赏赐、任命太守）
  - 新增对话框与提示系统
  - 完善战斗系统（单位选择、移动、攻击、自动战斗、AI）
  - 修复 quit 方法定义缺失的 bug
